# ValueShield — A/H 股价值投资管家 v2.6.3
...
- **代理免疫**：持久化 `_SESSION`（HTTPAdapter 5次退避重试）+ 东财 Referer Headers；`_no_proxy_ctx` 双阶段策略（有代理先走代理，宇宙为空自动切直连重试）；种子预热确认网络可用；WAF 降频 1.5s 间隔
...
**384 passed，10 deselected**（smoke 测试需真实网络，通过 `-m "not smoke"` 自动排除）
