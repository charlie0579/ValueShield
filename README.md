# ValueShield — A/H 股价值投资管家 v2.6.5
...
### 10. ✨ 神奇公式全市场扫描器（v2.5 起 · v2.6.x 持续强化）
...
- **代理免疫**：持久化 `_SESSION`（HTTPAdapter 5次退避重试）+ 东财 Referer Headers；**直连优先策略**（种子预热直连成功→全程直连）；东财/新浪域名白名单强制直连；种子预热门控；WAF 降频 1.5s 间隔
...
```
ValueShield/
├── app.py                    # Streamlit Web 看板（v2.6.5，双模式导航 + 扫描进度断点恢复）
...
├── magic_formula.py          # 神奇公式扫描器（ROC+EY 双因子，A+H 股，持久化Session+WAF降频+种子预热+双阶段代理策略）
...
**部署支持：** Mac/Linux (`setup_mac.sh` / `start_mac.sh`) · Windows (`setup_win.ps1` / `start_win.ps1`)
...
# 编辑配置（填入 Bark Token 和局域网 IP）
vim config.json

# 启动 Web 看板（默认 8501 端口）
bash start_mac.sh

# 指定端口
bash start_mac.sh --port 8502
```

> 💡 `start_mac.sh` 内部使用 `python -m streamlit run app.py`，
> 可彻底规避 Mac/Linux 上 `command not found: streamlit` 的 PATH 问题。
...
# 启动（默认 8501 端口）
powershell -ExecutionPolicy Bypass -File start_win.ps1

# 指定端口
powershell -ExecutionPolicy Bypass -File start_win.ps1 -Port 8502
```
...
> ⚠️ **首次部署**：需先让 `monitor.py` 完成至少一次轮询。
> 若单独打开 app.py，页面会显示引导提示，不会崩溃。
...
**392 passed，10 deselected**（smoke 测试需真实网络，通过 `-m "not smoke"` 自动排除）
...
> v2.6.5 为部署脚本与依赖清单版本，不含新增业务逻辑，无需补充 UT。
> 累计：**392 passed，10 deselected**

### v2.6.4 新增测试（+8）

| 类 | 文件 | 数 | 覆盖场景 |
|---|---|---|---|
| `TestDirectConnectDomains` | test_magic_formula.py | 6 | eastmoney/sinajs 白名单、普通域名排除、proxies路由逻辑（标志开/关）|
| `TestCheckNetworkConnectivityDirectFirst` | test_magic_formula.py | 2 | 直连成功→设 _DIRECT_CONNECT_PREFERRED=True、仅代理成功→False |

### v2.6.3 新增测试（+8）

| 类 | 文件 | 数 | 覆盖场景 |
|---|---|---|---|
| `TestPersistentSession` | test_magic_formula.py | 3 | Session 实例类型、http/https 均挂载重试 Adapter(total=5) |
| `TestCheckNetworkConnectivity` | test_magic_formula.py | 3 | 200返回True、全失败返回False、代理失败直连兜底 |
| `TestFetchDelay` | test_magic_formula.py | 1 | _FETCH_DELAY ≥ 1.5s（WAF 降频断言）|
| `TestNetworkGating` | test_magic_formula.py | 1 | 网络不通时快速返回 error=network_unavailable |
...
| `TestNoProxyCtx` | test_magic_formula.py | 4 | 清除代理变量生效、退出后恢复、异常时也恢复(RAII)、无代理时静默无副作用 |
...
| 版本 | 核心改动 |
|------|---------|
| **v2.6.5** | 🚀 全平台一键部署：`setup_mac.sh`/`start_mac.sh`/`setup_win.ps1`/`start_win.ps1` · `python -m streamlit` 规避 PATH 问题 · `requirements.txt` 规范化（pytest-mock/urllib3）|
| **v2.6.4** | 🔌 直连优先策略：`check_network_connectivity` 直连成功→全程强制直连 · 东财/新浪域名白名单 `_DIRECT_CONNECT_DOMAINS` · `_get_proxies_for_url` 路由决策 · SyntaxWarning 修复 · +8 条 UT（392 passed） |
| **v2.6.3** | 🌐 Linux WAF 穿透：持久化 `_SESSION`（HTTPAdapter 5次退避重试）· 东财 Referer Headers · WAF 降频 1.5s · `check_network_connectivity` 种子预热门控 · UI 网络不可用提示 · +8 条 UT（384 passed） |
| **v2.6.2** | 🛡️ 代理免疫力：`_no_proxy_ctx` 双阶段扫描策略（有代理→自动降级直连）· `_proxy_resilient_get` 3 次重试 · 金融黑名单静态兜底（40 只）· 扫描进度断点恢复 · smoke 整理 deselected 16→10 · +10 条 UT（376 passed） |
...
- [x] **Linux WAF 穿透**：持久化 Session + 东财 Headers + 降频 1.5s + 种子预热门控（v2.6.3 ✅）
- [x] **直连优先策略**：种子预热直连成功全程强制直连 + 东财/新浪域名白名单（v2.6.4 ✅）
...
### ⚙️ 部署层面
- [x] **全平台一键脚本**：Mac/Linux sh + Windows ps1（v2.6.5 ✅）
