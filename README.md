...
- **代理免疫**：`_no_proxy_ctx` 双阶段策略（有代理先走代理，宇宙为空自动切直连重试）；`_proxy_resilient_get` 3 次重试；金融黑名单网络失败回退 40 只静态兜底
...
├── magic_formula.py          # 神奇公式扫描器（ROC+EY 双因子，A+H 股，_no_proxy_ctx 双阶段代理策略，静态金融黑名单兜底）
...
**376 passed，10 deselected**（smoke 测试需真实网络，通过 `-m "not smoke"` 自动排除）
